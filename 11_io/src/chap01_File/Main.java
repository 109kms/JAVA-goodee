 ...
 *      (2) Windows에서 개발할때로 슬래시(/)를 사용합니다. (대부분 슬래시, 역슬래시 모두 경로 구분자로 인식하도록 호환성을 제공합니다.)
 * 2. 예약된 디렉터리
 *    1) 현재 디렉터리 : .
 *    2) 상위 디렉터리 : ..
 *    3) 루트 디렉터리 : /
...
public class Main {
  ...
  public static void handling() {
    ...
    //----- 실습. 현재날짜를 디렉터리 이름으로 사용해서 생성하시오. (예: C:/2025/07/10)
    File dir1 = new File("C:/" + DateTimeFormatter.ofPattern("yyyy/MM/dd").format(LocalDate.now()));
  ...
  public static void deleteDirectoryTree(String pathname) {
    ...
    File dir = new File(pathname);  //-- 최초 호출 시 "C:/2025/07/10" 경로
                                    //-- 다음 호출 시 "C:/2025/07" 경로
                                    //-- 다음 호출 시 "C:/2025" 경로
                                    //-- 다음 호출 시 "C:" 경로(루트 경로로 인식 : 부모 디렉터리가 없는 시점이 종료 시점)
...
//     handling();
    deleteDirectoryTree("C:/2025/07/10");  // 실습. C:/2025/07/10 디렉터리를 모두 삭제하세요
//    information();
