...
public class Main {
  ...
  public static void forEach() throws Exception{
    ...
    //----- Stream 생성
    Stream<String> s1 = Stream.of("봄", "여름", "가을", "겨울");

    //----- void forEach(Consumer<T> c);
    s1.forEach(season -> System.out.println(season));  //- 람다 표현식

    //----- Stream 사용은 일회성이므로 다시 사용하려면 다시 생성해야 합니다
    s1 = Stream.of("봄", "여름", "가을", "겨울");
    s1.forEach(System.out::println);  //------------------ 메소드 참조
  ...
  public static void map() throws Exception {
    ...
    //----- 지금까지 과정을 물 흐르듯(stream) 한 번에 처리해 봅시다. (메소드 참조)
    Stream.of("spring", "summer", "autume", "winter")  //----- 1. Stream 생성
    .map(String::toUpperCase)  //----------------------------- 2. 중간 연산
    .forEach(System.out::println);  //------------------------ 3. 최종 연산

    // 람다
    Stream.of("spring", "summer", "autume", "winter")  //----- 1. Stream 생성
    .map(season -> season.toUpperCase())  //----------------------------- 2. 중간 연산
  ...
  public static void filter() throws Exception {
    ...
    //----- Stream 생성
    IntStream s = IntStream.of(-1, 1, 2, -2, 3, -5, -3, 4, 5, -4);
  ...
  public static void listStream() throws Exception {
    ...
    fruits.add(new Fruit("Peach", 300.0));
    fruits.add(new Fruit("Strawberry", 50.0));
    fruits.add(new Fruit("Blueberry", 30.0));
  ...
  public static void fileStream() throws Exception {
    ...
    long sum = Arrays.stream(files)
        ...
        .mapToLong(file -> file.length())  //----- Stream은 sum() 지원이 안되므로 LongStream으로 바꾼 뒤 sum() 호출합니다.
        .sum();
    System.out.println("모든 파일 크기의 합: " + sum + "byte");
